...
dataset_simulation <- sim.data(20,c(0,0,0))
...
# Added these back commands back in, needed below
nDelta <- 10
...
# The for loop below does not work because no data is being simulated
# On each iteration of the for loop, new data should be simulated
N <- 50
n.var <- 5
pen_type <- "ToZero"
for (j in 1:nDelta) {
  ...
  mle.est <- matrix(rep(0,nDelta*K),nrow = K)
  pen.est <- matrix(rep(0,nDelta*K),nrow = K)
  mu.true <- seq(0,delta.grid[j], length.out = n.var)
  ...
  for (k in 1:K) {
    ...
    # data.opt <- list(dataset_simulation, lambda = lambda.out)
    mu.out <- optim(par = mu.mle,fn = pen_dist, data = dataset_simulation, lambda = lambda.out, pen_type = pen_type, method = "BFGS")$par
    mle.est[k,] <- mu.mle
    pen.est[k,] <- mu.out
  }
